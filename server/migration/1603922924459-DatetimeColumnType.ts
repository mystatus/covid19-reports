...
export class DatetimeColumnType1603922924459 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE TYPE "custom_roster_column_type_enum" AS ENUM('string', 'boolean', 'date', 'datetime', 'number')`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE TYPE "custom_roster_column_type_enum_old" AS ENUM('string', 'boolean', 'date', 'number')`);
    await queryRunner.query(`ALTER TABLE "custom_roster_column" ALTER COLUMN "type" DROP DEFAULT`);
