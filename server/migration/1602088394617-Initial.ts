...
export class Initial1602088394617 implements MigrationInterface {
  name = 'Initial1602088394617';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE TABLE "role" ("id" SERIAL NOT NULL, "name" character varying(2048) NOT NULL, "description" character varying(2048) NOT NULL, "index_prefix" character varying NOT NULL DEFAULT '', "allowed_roster_columns" character varying NOT NULL DEFAULT '', "notify_on_access_request" boolean NOT NULL DEFAULT false, "can_manage_group" boolean NOT NULL DEFAULT false, "can_manage_roster" boolean NOT NULL DEFAULT false, "can_view_roster" boolean NOT NULL DEFAULT false, "can_view_muster" boolean NOT NULL DEFAULT false, "can_view_pii" boolean NOT NULL DEFAULT false, "can_manage_workspace" boolean NOT NULL DEFAULT false, "org_id" integer NOT NULL, CONSTRAINT "PK_b36bcfe02fc8de3c57a8b2391c2" PRIMARY KEY ("id"))`);
    ...
    await queryRunner.query(`CREATE TABLE "roster" ("edipi" character varying(10) NOT NULL, "first_name" character varying(100) NOT NULL, "last_name" character varying(100) NOT NULL, "unit" character varying(50) NOT NULL, "billet_workcenter" character varying(50) NOT NULL, "contract_number" character varying(100) NOT NULL, "rate_rank" character varying(100), "pilot" boolean NOT NULL DEFAULT false, "aircrew" boolean NOT NULL DEFAULT false, "cdi" boolean NOT NULL DEFAULT false, "cdqar" boolean NOT NULL DEFAULT false, "dscacrew" boolean NOT NULL DEFAULT false, "advanced_party" boolean NOT NULL DEFAULT false, "pui" boolean NOT NULL DEFAULT false, "covid19_test_return_date" TIMESTAMP, "rom" character varying(50), "rom_release" character varying(100), "last_reported" TIMESTAMP DEFAULT null, "org_id" integer NOT NULL, CONSTRAINT "PK_6d3bc54502350051de7e30cfb91" PRIMARY KEY ("edipi", "org_id"))`);
    ...
    await queryRunner.query(`ALTER TABLE "user_roles" ADD CONSTRAINT "FK_781a1c15149789c1609fe1b0258" FOREIGN KEY ("user") REFERENCES "user"("edipi") ON DELETE CASCADE ON UPDATE NO ACTION`);
    await queryRunner.query(`ALTER TABLE "user_roles" ADD CONSTRAINT "FK_0475850442d60bd704c58041551" FOREIGN KEY ("role") REFERENCES "role"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
  }
